...
gettraveltimes <- function(cntyst){
  ...
  if(!file.exists(paste0("./R/DATA-PROCESSED/COMMUTINGCOSTS/h", cntyst,".rds"))){
    ...
    weighted_coords <-  cbind(centroids$lon, centroids$lat) %>%
      as_tibble() %>% setNames(c("lon","lat"))
    # pointCoords <- st_transform(cc$osm_points[,c("osm_id", "geometry")], crs = st_crs(blocks))
    coords <- st_as_sf(pointCoords,
                       coords = c("x_", "y_"),
                       crs = crs(allcounties))
    nodes_coords <-  do.call(rbind, st_geometry(coords)) %>%
    ...
    breaks <- seq(1, nrow(centroids), length.out= 5)
    to1 <- centroids$nodes[breaks[1]: breaks[2]]
    to2 <- centroids$nodes[breaks[2]: breaks[3]]
    to3 <- centroids$nodes[breaks[3]: breaks[4]]
    to4 <- centroids$nodes[breaks[4]: breaks[5]]
    ...
    for(i in 1:4){
      ...
      to <- get(paste0("to", i))
      print(paste("8/9 Getting Travel Times.", i))
      ...
      times2 <- times2[pairs %in% h$pairs,c(3:16)]
      ...
      z2060 <- z3[year %in% c(2015:2017), .(commuters = mean(commuters), 
      ...
      saveRDS(z3, file = paste0("./R/DATA-PROCESSED/COMMUTINGCOSTS/h",cntyst,"_",i,".rds")) 
...
targetcounties <- sort(unique(commutingareas$h_county[which(
  commutingareas$h_state %in% c("36") ｜commutingareas$h_county %in% c("06037", 
                                                                      "06059", "06111"))]))
